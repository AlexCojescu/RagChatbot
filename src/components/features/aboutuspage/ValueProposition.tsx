...
const DifferentiatorCard: React.FC<DifferentiatorCardProps> = ({ 
  ...
  const colorClasses = {
    blue: {
      ...
      hover: 'hover:border-white/40 hover:shadow-blue-500/20 group-hover:bg-white/30'
    },
    purple: {
      bg: 'bg-white/20',
      text: 'text-purple-600',
      border: 'border-white/30',
      hover: 'hover:border-white/40 hover:shadow-purple-500/20 group-hover:bg-white/30'
    },
    emerald: {
      bg: 'bg-white/20',
      text: 'text-emerald-600',
      border: 'border-white/30',
      hover: 'hover:border-white/40 hover:shadow-emerald-500/20 group-hover:bg-white/30'
    }
  };
  ...
  return (
    <div className={`group relative overflow-hidden rounded-2xl border ${colors.border} bg-white/15 backdrop-blur-sm p-8 lg:p-10 shadow-sm transition-all duration-500 ease-out hover:shadow-xl hover:-translate-y-1 ${colors.hover}`}>
      ...
      <div className="relative container mx-auto px-6 sm:px-8 lg:px-12 max-w-7xl">
        ...
        <div className="text-center mb-20 lg:mb-24">
          <div className="space-y-8 max-w-5xl mx-auto">
            <div className="inline-flex items-center rounded-full border border-white/40 bg-white/20 backdrop-blur-sm px-5 py-3 text-sm font-semibold text-indigo-700 shadow-sm">
        ...
        <div className="mb-24 lg:mb-32">
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 max-w-6xl mx-auto">
            {valuePoints.map((point, index) => (
              <div 
                key={index} 
                className="group relative overflow-hidden bg-white/15 backdrop-blur-sm rounded-2xl p-8 shadow-sm border border-white/30 transition-all duration-500 ease-out hover:bg-white/25 hover:shadow-lg hover:-translate-y-0.5 opacity-0 animate-[fadeInUp_0.6s_ease-out_forwards]"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div className="relative flex flex-col items-center text-center space-y-4">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/25 text-indigo-600 transition-all duration-300 group-hover:scale-110 group-hover:bg-white/35">
                    {point.icon}
                  </div>
        ...
        <div className="space-y-16 lg:space-y-20">
          <div className="text-center space-y-8 max-w-4xl mx-auto">
            <div className="inline-flex items-center rounded-full border border-white/40 bg-white/20 backdrop-blur-sm px-5 py-3 text-sm font-semibold text-gray-700 shadow-sm">
