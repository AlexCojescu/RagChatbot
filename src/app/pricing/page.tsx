    ...
    <>      
      <div className="relative min-h-screen">
        ...
        <div className="relative w-full pt-8 pb-4">
          ...
          {/* Filter Navigation Buttons with glassmorphism styling */}
          <div className="flex justify-center items-center space-x-2 sm:space-x-4 mb-8 px-4 flex-wrap">
            {sections.map((section, index) => (
              <button
                key={section.title}
                onClick={() => goToSlide(index)}
                className={`py-2 px-4 my-1 rounded-full text-sm sm:text-base font-semibold transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 backdrop-blur-sm border ${
                  currentIndex === index
                    ? 'bg-blue-600 text-white shadow-md border-blue-600'
                    : 'bg-white/20 text-gray-800 hover:bg-white/30 border-white/30'
                }`}
          ...
          <main className="relative w-full">
            ...
            <button
              ...
              className="absolute top-1/2 left-2 sm:left-4 md:left-6 lg:left-8 z-10 -translate-y-1/2 p-2 rounded-full bg-white/30 backdrop-blur-sm hover:bg-white/40 transition-colors shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 border border-white/30"
            ...
            <button
              onClick={goToNext}
              className="absolute top-1/2 right-2 sm:right-4 md:right-6 lg:right-8 z-20 -translate-y-1/2 p-2 rounded-full bg-white/30 backdrop-blur-sm hover:bg-white/40 transition-colors shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 border border-white/30"
